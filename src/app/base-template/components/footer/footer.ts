...
export class TemplateFooter {
  ...
  readonly restaurantName = computed(
    () => this._restaurantService.restaurant()?.name ?? 'TU RESTAURANTE',
  );
  readonly restaurantLogo = computed(() => this._restaurantService.settings()?.logo_url ?? '');
  readonly restaurantDescription = computed(
